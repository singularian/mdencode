...
func mddecode() int {
	...
	md.initLog()
        ...
        if argsNumber == 3 {
                ...
                modsize    := os.Args[2]
                blocksizeint, _ := strconv.Atoi(blocksize)
                md.modulusScanRandom(blocksizeint, modsize)
        // test random
        } else {
...
func (md *DecodeData) modulusScanRandom(blockSize int, modSize string) {
...
func (md *DecodeData) decode(blockSize string, modbitSize string, modexp string, remainder string, md5hex string, sha1hex string) (int, string) {
