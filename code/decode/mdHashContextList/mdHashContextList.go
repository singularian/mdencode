...
func (hc *HashContextList) CreateHashListMap(hashList string, mdtype int, threadNumber int) {
	...
	// for thread := 0; thread < threadNumber; thread++ {
		...
		switch hashname {
			...
			case "mar32":
				var key = hc.keylist[hashname]
				hb["mar32"] = marvin32.New(0, 4, sigRand.ConvertString2Int(key))
			// meow hash 32, 64 and 128
			case "me32":
				var key = hc.keylist[hashname]
			...
			case "met64":
				var key = hc.keylist[hashname]
				hb["met64"] = metro64.New(0, 8, sigRand.ConvertString2Int(key))
			// murmur3 32
			case "mm32":
				var key = hc.keylist[hashname]
			...
			case "t1ha":
				var key = hc.keylist[hashname]
				hb["t1ha"] = t1ha.New(0, 8, sigRand.ConvertString2Int(key))
                        case "whirlpool":
