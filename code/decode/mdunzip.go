...
func main() {
	...
	if argsNum == 2 {
		...
		fmt.Println(exe, " [FILENAME] [OUTPUTFILENAME]")
...
//		fmt.Println("block hash ", blockSize, hlistarray, int(modSize), n.String());
		...
		for thread = 0; thread < threadCount; thread++ {
			...
			md := modScanFile.Init(int64(currentBlocksize), int64(modSize), thread, threadCount, emptybytes, time)
			mdp = append(mdp, md)
		}
...
func callGo (blocknumber uint64, currentBlocksize int64, modSize int64, modSize2 uint32, hashlist string, modRemainder string  ) {
		...
		// var xbytes = make([]byte, currentBlocksize)
                ...
                var emptybytes []byte
                for thread = 0; thread < threadCount; thread++ {
                        //md := modScanFile.Init(blockSizeInt, modSizeInt, thread, threadCount, bytes, time)
                        md := modScanFile.Init(int64(currentBlocksize), int64(modSize), thread, threadCount, emptybytes, time)
                        mdp = append(mdp, md)
                }
