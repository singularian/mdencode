...
// package mdtestHC.go
//
// mdtest is a test program for the parallel modulus scan on a random generated byte block
// mdtest uses the hash context list 
//
// mdtest creates the byte block randomly with n-bytes and creates an n-bit modular floor
// mdtest will then run a parallel modulus scan to find the original n-byte block with a matching signature
// mdtest uses goroutines to run the scans in sequence and channels to communicate
//
...
func main() {
	...
	// fmt.Println("bhashlist ", fd.bhashlist, fd.blocksize)
...
func (fd *FlagData) mddecode(blocksize string, modsize string, blocklist string, threadsize string, threadlist string, bytestring string, hexstring string) int {
	...
	if len(blockHlist) < 1 {
		fmt.Println("Unrecognized boolean or csv hashlist")
		fmt.Println("No hashlist found ", blockHlist)
		os.Exit(1)
	}
