...
func (md *MdFormat) EncodeFileHeader(encodingFormat int, fileName string, filePath string, fileSize int64, blockSize int64, filehashList []string, blockhashList []string, modulusSize int64) {
	...
	case 0:
                var mdhead  = fmt.Sprintf("%s\n%s\n%v:%v:%v:%v:%s-%s", fileName, filePath, fileSize, encodingFormat, blockSize, modulusSize, filehashListString, blockhashListString)
                buffer.WriteString(mdhead)
	case 4:
                var mdhead  = fmt.Sprintf("%s\n%s\n%v:%v:%v:%v:%s-%s:%v", fileName, filePath, fileSize, encodingFormat, blockSize, modulusSize, filehashListString, blockhashListString, time.Now())
                buffer.WriteString(mdhead)
	case 5:
                var mdhead  = fmt.Sprintf("%s\n%s\n%v:%v:%v:%v:%s-%s:%v", fileName, filePath, fileSize, encodingFormat, blockSize, modulusSize, filehashListString, blockhashListString, time.Now())
                buffer.WriteString(mdhead)
	case 7:
                var mdhead  = fmt.Sprintf("%s\n%s\n%v:%v:%v:%v:%s-%s:%v:%v", fileName, filePath, fileSize, encodingFormat, blockSize, modulusSize, filehashListString, blockhashListString, md.mdVersion, time.Now())
                buffer.WriteString(mdhead)
	default:
		encodingFormat = 10 
		var mdhead  = fmt.Sprintf("filename: %s\nfilepath: %s\nheader: %v:%v:%v:%v:%s-%s:%v:%v", fileName, filePath, fileSize, encodingFormat, blockSize, modulusSize, filehashListString, blockhashListString, md.mdVersion, time.Now())
		buffer.WriteString(mdhead)
	}
